...
class AlbumLikeView(APIView):
    ...
    def post(self, request, album_id):
        ...
        if user.role == "singer":
            return Response({"detail": "Singers cannot like albums"}, status=status.HTTP_403_FORBIDDEN)
...
class AlbumDislikeView(APIView):
    ...
    def post(self, request, album_id):
        ...
        if album.singer == user:
            return Response(
                {"detail": "You cannot dislike your own album"},
...
class AlbumSaveLibraryView(APIView):
    ...
    def post(self, request, album_id):
        ...
        if user in album.saved_by.all():
            ...
            message = "album removed from your playlist"
        else:
            album.saved_by.add(user)
            message = "album saved to your playlist"
